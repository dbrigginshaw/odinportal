...
function countdown_init(seconds) {
    countdown_number = seconds;
    countdown_trigger();
}
...
function countdown_trigger() {
    if(countdown_number > 0) {
        countdown_number--;
        document.getElementById('TextBoxPasswordError').innerHTML = countdown_number;
        if(countdown_number > 0) {
		...
		else
		{
			...
			if(document.getElementsByName('TextBoxUserId')[0].value == "admin")
			{
...
//	clearTimeout();		// not valid in NS6
	...
	{
		countdown_init(password.substr(4));
		return;
	}
	if(password.substr(0, 6) == "waitv1")
	{
		wait(password.substr(6));
	}
